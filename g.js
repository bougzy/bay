...
app.get("/api/admin/users", adminAuth, async (req, res) => {
  try {
    const users = await User.find()
      .select("name email walletBalance referrals isBlocked createdAt");
    res.json(users);
  } catch (e) {
    res.status(500).json({ message: e.message });
  }
...
// --- Admin: Get withdrawals for a user ---
...
// --- Admin: Add profit (log + update balance) ---
app.put("/api/admin/user/:userId/profit", adminAuth, async (req, res) => {
  try {
    const { amount, note } = req.body;
    const user = await User.findById(req.params.userId);
    if (!user) return res.status(404).json({ message: "User not found" });
    ...
    res.json({ message: "Profit added", profit });
  } catch (e) {
    res.status(500).json({ message: e.message });
  }
});

// --- Admin: Get profit history for a user ---
